...
class Movie
{
     ...
     */
    public function fillImdb($imdbmovie, $parental_guidance)
    {
    ...
    public function fillTmdb($tmdbmovie, $parental_guidance)
    {
        $this->id = isset($this->id) ? $this->id : 0;
        $this->tmdbid = $this->htmldecode($tmdbmovie->getID());
        $this->name = $this->htmldecode($tmdbmovie->getTitle());
        $this->nameorder = isset($this->nameorder) ? $this->nameorder : "";


        $akas = [];
        foreach ($tmdbmovie->get('translations')['translations'] as $aka) {
            $title = $aka['data']['title'];
            if (empty($title)) continue;
            $akas[] = $this->htmldecode($aka['data']['title']);
        }
        $this->aka = join("\n", $akas);
        $this->year = $this->htmldecode(date('Y', strtotime($tmdbmovie->get('release_date'))));
        $this->duration = $this->htmldecode($tmdbmovie->get('runtime'));
        $this->rating = $this->htmldecode($tmdbmovie->get('vote_average'));
        $this->votes = $this->htmldecode($tmdbmovie->getVoteCount());
        //$age = $this->parentalGuidance($imdbmovie->mpaa(),$parental_guidance);
        $this->pg = 0;
        $this->favourite = isset($this->favourite) ? $this->favourite : false;
        $this->own = isset($this->own) ? $this->own : true;
        ...
        $this->trailer = isset($this->trailer) ? $this->trailer : "";
        $this->notes = isset($this->notes) ? $this->notes : "";
        $this->taglines = $this->join("\n\n", $this->htmldecode($tmdbmovie->get('tagline')));

        $this->plotoutline = trim(strip_tags($this->htmldecode($tmdbmovie->get('overview'))));
        $this->plots = $this->join("\n\n", [$this->htmldecode($tmdbmovie->get('overview'))]);
        $languages = [];
        foreach ($tmdbmovie->get('spoken_languages') as $lng) {
            $languages[] = $lng["english_name"];
        }

        $this->languages = $this->join("\n", $this->htmldecode($languages));
        $this->subtitles = isset($this->subtitles) ? $this->subtitles : "";
        $this->audio = isset($this->audio) ? $this->audio : "";
        $this->video = isset($this->video) ? $this->video : "";

        $countries = [];
        foreach ($tmdbmovie->get('production_countries') as $country) {
            $countries[] = $country['iso_3166_1'];
        }
        $this->country = $this->join("\n", $this->htmldecode($countries));

        $genres = array();
        foreach ($tmdbmovie->getGenres() as $genre) {
            $genres[] = $genre->getName();
        }

        $this->genres = $this->join("\n", $this->htmldecode($genres));
        //$this->mpaa = $this->join("\n", $this->htmldecode($imdbmovie->mpaa()), '::');
        ...
        $director = [];
        $writer = [];
        foreach ($tmdbmovie->get('credits')['crew'] as $crew) {
        ...
        $this->director = $this->join("\n", $this->htmldecode(array_unique($director)));
        $this->writer = $this->join("\n", $this->htmldecode(array_unique($writer)));
        $this->producer = $this->join("\n", $this->htmldecode(array_unique($production)));
        $this->music = $this->join("\n", $this->htmldecode(array_unique($sounds)));

        $casts = [];
        foreach ($tmdbmovie->getCast() as $cast) {
            $casts[] = $cast->getName();
        }
        $this->cast = $this->join("\n", $this->htmldecode($casts));
    }
