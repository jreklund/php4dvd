 ...
 * 		Search TMDBMovie		http://api.themoviedb.org/3/search/movie
 * 		Search Person		http://api.themoviedb.org/3/search/person
 * 		TMDBMovie Info			http://api.themoviedb.org/3/movie/11
...
require ("controller/classes/data/TMDBMovie.php");
...
class TMDB {
	 ...
	 */
	public function getLatestMovie() {
		return new TMDBMovie($this->_call('movie/latest'));
	}
	 ...
	 * 	Get a TMDBMovie
	 *
	 * 	@param int $idMovie The TMDBMovie id
	...
	public function getMovie($idMovie, $appendToResponse = null) {
		...
		return new TMDBMovie($this->_call('movie/' . $idMovie, $appendToResponse));
	}
    ...
    {
        $searchResults = array(
            TMDBMovie::MEDIA_TYPE_MOVIE => array(),
        ...
        foreach ($result['results'] as $data) {
            if ($data['media_type'] === TMDBMovie::MEDIA_TYPE_MOVIE) {
                $searchResults[TMDBMovie::MEDIA_TYPE_MOVIE][] = new TMDBMovie($data);
	 ...
	 *  Search TMDBMovie
	 *
	 * 	@param string $movieTitle The title of a TMDBMovie
	 * 	@return TMDBMovie[]
	 */
	public function searchMovie($movieTitle){
	...
	public function find($id, $external_source = 'imdb_id'){
		...
		foreach($result['movie_results'] as $data){
			$found['movies'][] = new TMDBMovie($data);
		}
